...
function extractSimpleChatObservability(messages) {
  ...
  for (const msg of messages || []) {
    ...
    if (Array.isArray(msg.comparisons)) {
      msg.comparisons.forEach((row) => {
        if (row?.usage && typeof row.usage === "object") {
          llmTraces.push({
            ts: msg.ts || null,
            type: "assistant-comparison",
            model: row.model || null,
            usage: extractUsage({ usage: row.usage })
          });
        }
        payloadTraces.push({
          ts: msg.ts || null,
          type: "assistant-comparison",
          model: row?.model || null,
          responsePreview: truncateMasked(row?.error ? `Error: ${row.error}` : row?.content || "", 900)
        });
      });
    }
...
function summarizeDebate({ debateId, session, transcript, messageRows }) {
  ...
  return {
    ...
    deleteMode: session.deleteMode || null,
...
function summarizeSimpleChat({ chatId, session, messages }) {
  ...
  const comparisonRows = assistantMessages.flatMap((m) =>
    Array.isArray(m.comparisons)
      ? m.comparisons.map((row) => ({
          ...row,
          ts: m.ts || null
        }))
      : []
  ...
  const primaryUsageTotals = assistantMessages.reduce(
    (acc, row) => {
  ...
  const comparisonUsageTotals = comparisonRows.reduce(
    (acc, row) => {
      const usage = extractUsage({ usage: row.usage || {} });
      acc.promptTokens += usage.promptTokens;
      acc.completionTokens += usage.completionTokens;
  ...
  const usageTotals = {
    promptTokens: primaryUsageTotals.promptTokens + comparisonUsageTotals.promptTokens,
    completionTokens: primaryUsageTotals.completionTokens + comparisonUsageTotals.completionTokens,
    totalTokens: primaryUsageTotals.totalTokens + comparisonUsageTotals.totalTokens
  };
  const model = session?.settings?.model || "unknown";
  const modelBreakdownMap = new Map();
  function appendModelUsage(modelName, usage, responseCount = 1) {
    const key = String(modelName || "unknown");
    if (!modelBreakdownMap.has(key)) {
    ...
    const row = modelBreakdownMap.get(key);
    row.responseCount += responseCount;
    row.tokenUsage.promptTokens += Number(usage.promptTokens || 0);
    row.tokenUsage.completionTokens += Number(usage.completionTokens || 0);
    row.tokenUsage.totalTokens += Number(usage.totalTokens || 0);
  }
  assistantMessages.forEach((row) => {
    appendModelUsage(row.model || model, extractUsage({ usage: row.usage || {} }), 1);
  });
  comparisonRows.forEach((row) => {
    appendModelUsage(row.model || "unknown", extractUsage({ usage: row.usage || {} }), 1);
  });
  const modelBreakdown = [...modelBreakdownMap.values()].map((row) => {
    const modelCost = estimateCostUsd(row.model, row.tokenUsage);
    return {
      ...row,
      estimatedCostUsd: modelCost.estimatedCostUsd,
      pricingKnown: modelCost.pricingKnown
    };
  });
  const knownCosts = modelBreakdown
    .map((row) => row.estimatedCostUsd)
    .filter((value) => typeof value === "number");
  const totalEstimatedCostUsd = knownCosts.length
    ? Number(knownCosts.reduce((acc, value) => acc + Number(value || 0), 0).toFixed(6))
    : null;
  const pricingKnown = modelBreakdown.length > 0 && modelBreakdown.every((row) => row.pricingKnown);
  const comparisonModels = [...new Set(comparisonRows.map((row) => String(row.model || "").trim()).filter(Boolean))];
  const risk = aggregateRiskSignals(messages.map((m) => m.content));
  ...
  return {
    ...
    model,
    modelsInSession: [model, ...comparisonModels.filter((name) => name !== model)],
    modelBreakdown,
    comparisonModelCount: comparisonModels.length,
    comparisonRunCount: comparisonRows.length,
    ...
    pricingKnown,
    primaryOnlyTokenUsage: primaryUsageTotals,
    comparisonTokenUsage: comparisonUsageTotals,
