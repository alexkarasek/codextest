...
export const AGENTIC_WORKFLOWS_DIR = path.join(AGENTIC_DIR, "workflows");
export const AGENTIC_WORKFLOW_RUNS_DIR = path.join(AGENTIC_DIR, "workflow-runs");
...
export async function ensureDataDirs() {
  ...
  await fs.mkdir(AGENTIC_WORKFLOWS_DIR, { recursive: true });
  await fs.mkdir(AGENTIC_WORKFLOW_RUNS_DIR, { recursive: true });
}
...
export function personaToMarkdown(persona) {
  ...
  const mcpServerIds = (persona.mcpServerIds || []).join(", ");
