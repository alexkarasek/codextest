...
import { isHttpMcpServer, listHttpMcpTools } from "./mcpHttpClient.js";
import { isStdioMcpServer, listStdioMcpTools } from "./mcpStdioClient.js";
...
function normalizeConfiguredServer(server) {
  ...
  return {
    ...
    source: "configured",
    endpoint: typeof server.endpoint === "string" ? server.endpoint : undefined,
    ...
    timeout_ms: Number.isFinite(Number(server.timeout_ms)) ? Number(server.timeout_ms) : undefined,
    protocol_version: typeof server.protocol_version === "string" ? server.protocol_version : undefined,
...
function mergeServerRecord(base, override) {
  ...
  return {
    ...
    url: base?.url || override?.url,
    command: base?.command || override?.command,
    args: base?.args || override?.args,
    cwd: base?.cwd || override?.cwd,
    env: base?.env || override?.env,
    timeout_ms: base?.timeout_ms || override?.timeout_ms,
    protocol_version: base?.protocol_version || override?.protocol_version,
    headers: base?.headers || override?.headers,
    allow_remote: Boolean(base?.allow_remote || override?.allow_remote),
...
export async function listResolvedMcpServers({ includeTools = false } = {}) {
  ...
  const merged = embedded.map((server) => mergeServerRecord(server, configuredById.get(server.id)));
  const configuredOnly = configured.filter((server) => !embedded.some((row) => row.id === server.id));
  const servers = [...merged, ...configuredOnly];
  if (!includeTools) return servers;
  return Promise.all(
    servers.map(async (server) => {
      if (!isHttpMcpServer(server)) return server;
      try {
        const tools = await listHttpMcpTools(server);
        return {
          ...server,
          tools,
    ...
    Promise.all(
      resolved.map(async (server) => {
        if (!isStdioMcpServer(server)) return server;
        try {
          const tools = await listStdioMcpTools(server);
          return {
            ...server,
            tools,
            toolCount: tools.length
