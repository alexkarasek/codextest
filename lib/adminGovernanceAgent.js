...
import { runMcpTool } from "./mcpRegistry.js";
...
function compactChat(chat) {
  ...
  return {
    ...
    modelsInSession: chat.modelsInSession || [chat.model || "unknown"],
    comparisonModelCount: Number(chat.comparisonModelCount || 0),
    comparisonRunCount: Number(chat.comparisonRunCount || 0),
  ...
  const session = {
    ...
    settings: {
      ...
      remoteMcpServerId: String(
        settings.remoteMcpServerId || process.env.GOVERNANCE_MCP_SERVER_ID || ""
      ).trim(),
      remoteMcpToolName: String(
        settings.remoteMcpToolName || process.env.GOVERNANCE_MCP_TOOL_NAME || "governance.chat"
      ).trim() || "governance.chat",
      engagementMode: "chat"
    },
    personas: [persona],
    knowledgeByPersona: {
...
function normalizeGovernanceMcpOutputToText(output) {
  if (output == null) return "";
  if (typeof output === "string") return output.trim();
  if (typeof output === "number" || typeof output === "boolean") return String(output);
  const direct = String(output?.text || output?.message || output?.answer || "").trim();
...
function summarizeObjectKeys(value) {
  ...
  return `I received structured governance data with keys: ${keys.join(", ")}.`;
}

async function formatGovernanceMcpOutput({
  session,
  userMessage,
  output
}) {
  const direct = normalizeGovernanceMcpOutputToText(output);
  if (direct) return direct;
  if (output == null) return "";

  const maxWords = Number(session?.settings?.maxWordsPerTurn || 220);
  const serialized = truncateText(JSON.stringify(output, null, 2), 12000);
  try {
      ...
      messages: [
        {
          ...
          content: [
            ...
            "Convert structured tool output into a concise conversational answer.",
            "Use plain language and include the most relevant facts for the user question.",
            "If fields are missing, say what is missing.",
  ...
  return summarizeObjectKeys(output) || "I received structured governance data, but could not format it cleanly.";
}

async function tryRemoteGovernanceChat({ session, userMessage }) {
  const serverId = String(session?.settings?.remoteMcpServerId || "").trim();
  if (!serverId) return { text: null, source: "local_llm", reason: "remote_mcp_not_configured" };
  const toolName = String(session?.settings?.remoteMcpToolName || "governance.chat").trim() || "governance.chat";
  try {
    const output = await runMcpTool(
      ...
      { source: "governance-admin-chat" }
    );
    const text = await formatGovernanceMcpOutput({
      session,
      userMessage,
      output
    });
    if (text) return { text, source: "remote_mcp", reason: "" };
    return { text: null, source: "local_llm", reason: "remote_mcp_empty_response" };
  } catch (error) {
    return {
      text: null,
      source: "local_llm",
      reason: `remote_mcp_error:${String(error?.code || error?.message || "unknown")}`
    };
  }
}
...
export async function sendGovernanceAdminChatMessage(chatId, userMessage) {
  ...
  const promptHistory = [...history, userEntry];
  const remoteResult = await tryRemoteGovernanceChat({
    session,
    userMessage: text
  });
  const completion =
    remoteResult.text !== null
