...
import { routeTeam, resolveTaskParticipantsByIds } from "./teamRouter.js";
...
async function resolveRunPersonas({ requestedPersonaIds, maxAgents = 3, task = null }) {
  if (Array.isArray(requestedPersonaIds) && requestedPersonaIds.length) {
    return resolveTaskParticipantsByIds(requestedPersonaIds, { maxAgents });
  }
  const routedParticipants = Array.isArray(task?.routing?.selectedParticipants)
    ? task.routing.selectedParticipants
    : [];
  if (routedParticipants.length) {
    return routedParticipants.slice(0, Math.max(1, maxAgents));
  }
  const routedIds = Array.isArray(task?.routing?.selectedPersonaIds) ? task.routing.selectedPersonaIds : [];
  if (routedIds.length) {
    return resolveTaskParticipantsByIds(routedIds, { maxAgents });
  }
  ...
  return routed.selectedParticipants || routed.selectedPersonas || [];
}

async function completeAutonomousTurn({ persona, model, temperature, messages, input }) {
  if (persona?.participantType === "foundry-agent" && persona?.provider === "foundry") {
    const result = await invokeFoundryApplicationByName(persona.applicationName || persona.id, {
      input,
      context: {
        purpose: "agentic-autonomous-decision-chain",
...
export async function runAutonomousDecisionChainScenario(input = {}, context = {}) {
  ...
  for (let round = 1; round <= rounds; round += 1) {
    ...
    for (const persona of personas) {
      ...
      const completion = await completeAutonomousTurn({
        persona,
        model,
        temperature,
        messages,
        input: String(messages[1]?.content || "").trim()
      });
