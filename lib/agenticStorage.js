...
import {
  ...
  AGENTIC_JOBS_DIR,
  AGENTIC_WORKFLOWS_DIR,
  AGENTIC_WORKFLOW_RUNS_DIR,
...
const WORKFLOW_EVENTS_PATH = path.join(AGENTIC_DIR, "workflow-events.jsonl");
...
function workflowPath(workflowId) {
  return path.join(AGENTIC_WORKFLOWS_DIR, `${workflowId}.json`);
}

function workflowRunPath(workflowRunId) {
  return path.join(AGENTIC_WORKFLOW_RUNS_DIR, `${workflowRunId}.json`);
}

export async function saveWorkflow(workflow) {
  await ensureAgenticDirs();
  await writeJsonFile(workflowPath(workflow.id), workflow);
  return workflow;
}

export async function getWorkflow(workflowId) {
  return readJsonFile(workflowPath(workflowId));
}

export async function updateWorkflow(workflowId, updater) {
...
export async function listWorkflows() {
  ...
  let files = [];
  try {
    files = await fs.readdir(AGENTIC_WORKFLOWS_DIR);
  } catch (error) {
    if (error.code === "ENOENT") return [];
    throw error;
  }
  ...
  for (const file of files) {
    if (!file.endsWith(".json")) continue;
    try {
      rows.push(await readJsonFile(path.join(AGENTIC_WORKFLOWS_DIR, file)));
    } catch {
      // skip bad files
    }
  }
...
export async function deleteWorkflow(workflowId) {
  await ensureAgenticDirs();
  try {
    await fs.rm(workflowPath(workflowId), { force: true });
    return true;
  } catch (error) {
    if (error.code === "ENOENT") return false;
    throw error;
...
export async function saveWorkflowRun(run) {
  await ensureAgenticDirs();
  await writeJsonFile(workflowRunPath(run.id), run);
  return run;
}

export async function getWorkflowRun(runId) {
  return readJsonFile(workflowRunPath(runId));
}
...
export async function listWorkflowRuns({ workflowId = "", limit = 200 } = {}) {
  ...
  let files = [];
  try {
    files = await fs.readdir(AGENTIC_WORKFLOW_RUNS_DIR);
  } catch (error) {
    if (error.code === "ENOENT") return [];
    throw error;
  }
  ...
  for (const file of files) {
    if (!file.endsWith(".json")) continue;
    try {
      const row = await readJsonFile(path.join(AGENTIC_WORKFLOW_RUNS_DIR, file));
      if (workflowId && String(row.workflowId || "") !== String(workflowId)) continue;
      rows.push(row);
    } catch {
      // skip bad files
    }
...
export async function appendWorkflowEvent(event) {
  await ensureAgenticDirs();
  await appendJsonl(WORKFLOW_EVENTS_PATH, event);
}
...
export async function listWorkflowEvents(limit = 1000) {
  return readJsonl(WORKFLOW_EVENTS_PATH, { limit });
}
