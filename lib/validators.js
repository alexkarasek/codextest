...
export const selectedPersonaSchema = z.union([
  ...
  z.object({
    ...
    provider: z.string().min(1).default("foundry"),
    id: z.string().min(1)
  }),
  z.object({
...
export const personaChatSettingsSchema = z.object({
  ...
  compareModels: z.array(z.string().min(1)).max(4).optional().default([])
});
...
export const workflowTriggerSchema = z.object({
  type: z.enum(["manual", "cron", "webhook", "runCompleted"]).default("manual"),
...
export const workflowStepSchema = z.object({
  id: z.string().min(1).optional(),
  name: z.string().min(1).optional(),
  type: z.enum(["condition", "httpRequest", "sendMessage", "runDebate", "transform", "persistRecord"]),
  input: z.record(z.any()).optional().default({}),
  requiresApproval: z.boolean().optional().default(false)
});
...
export const workflowSchema = z.object({
  id: z
    .string()
    .min(1, "id is required")
    .regex(slugRegex, "id must be a slug like daily-debate-run"),
  name: z.string().min(1, "name is required"),
  enabled: z.boolean().optional().default(true),
  trigger: workflowTriggerSchema.default({ type: "manual", cron: "", event: "", secret: "" }),
  steps: z.array(workflowStepSchema).min(1, "At least one workflow step is required."),
  createdAt: z.string().optional(),
  updatedAt: z.string().optional()
});

export const createWorkflowSchema = workflowSchema.omit({ createdAt: true, updatedAt: true });
...
export const foundryApplicationSchema = z.object({
  id: z.string().min(1),
  applicationName: z.string().min(1),
  displayName: z.string().optional().default(""),
  description: z.string().optional().default(""),
  tags: z.array(z.string().min(1)).optional().default([]),
  routesModels: z.boolean().optional().default(false),
  version: z.string().optional().default("")
});

export const foundryApplicationsSchema = z.array(foundryApplicationSchema).default([]);
