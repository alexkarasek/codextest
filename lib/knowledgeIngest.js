...
async function extractRawText(file) {
  ...
  if (ext === ".txt" || ext === ".md") return { text: await extractFromTxt(file.buffer), method: "txt" };
  ...
  const err = new Error("Unsupported file type. Allowed: .txt, .md, .pdf, .jpg/.jpeg/.png, .doc, .docx");
...
export async function ingestUrlToKnowledgePack({
  url,
  id,
  title,
  description,
  ...
  if (!rawText) {
    const err = new Error("No readable text found at URL.");
    err.code = "EMPTY_EXTRACTED_TEXT";
    throw err;
  }

  const inferredTitle = title?.trim() || doc.title || doc.url;
  const inferredId = id?.trim() || slugify(inferredTitle) || `knowledge-${Date.now()}`;
  const normalizedTags = normalizeTags(tags);
  const sourceText = truncateWebText(rawText, 12000);
  const content = summarize
    ? await condenseToKnowledgeContent({
        title: inferredTitle,
        extractedText: sourceText
      })
    : truncateText(sourceText, 5000);

  return {
    pack: {
      id: inferredId,
      title: inferredTitle,
      description: description?.trim() || `Generated from ${doc.url}`,
      tags: normalizedTags,
      content,
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
