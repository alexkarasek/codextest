...
export async function optimizePersonaForDebate({ persona, existingPersonas, model = "gpt-4.1-mini" }) {
  ...
  if (!isMateriallyImproved(persona, second)) {
    const err = new Error("Optimizer output was too similar to the input persona.");
    err.code = "OPTIMIZER_WEAK_OUTPUT";
    throw err;
  }
  ...
  return {
    ...
    optimization: {
      applied: true,
      strictRewrite: true,
      changedFields: countChangedFields(persona, second),
