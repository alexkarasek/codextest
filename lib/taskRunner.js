...
function buildToolUsageMetadata(toolId, input) {
  ...
  if (toolId === "web.fetch") {
    ...
    metadata.path = String(input?.path || "");
  } else if (toolId === "read_json") {
    metadata.path = String(input?.path || "");
  } else if (toolId === "write_json") {
    metadata.path = String(input?.path || "");
  } else if (toolId === "write_text") {
    metadata.path = String(input?.path || "");
  } else if (toolId === "policy_gates") {
    metadata.hasIntakeObject = Boolean(input?.intake_obj && typeof input.intake_obj === "object");
...
async function executeStep(task, step, user) {
  ...
  if (step.type === "llm") {
    const model = String(step.model || task.settings?.model || "gpt-5-mini");
...
export async function createTaskDraft({
  ...
  const task = {
    ...
    routing: {
      selectedParticipants: routing.selectedParticipants || routing.selectedPersonas || [],
